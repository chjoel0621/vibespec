...
**실행 환경 지원.** 검증된 JSON과 자기완결 HTML, 안전 수정, 재기준, 지도, 병합을 모두 쓰는 전체 모드는 Node.js 18 이상, 설치된 스킬 디렉터리 읽기, 작업 폴더 쓰기 권한이 필요합니다. Claude Code와 파일시스템을 사용할 수 있는 Codex 작업은 보통 이 조건을 충족합니다. Node나 셸을 제공하지 않는 Cowork 작업은 축소 모드만 사용합니다. 이때 VibeSpec은 JSON과 수정하지 않은 뷰어를 제공하고 사용자가 뷰어에서 JSON을 불러옵니다. 검증된 embedded HTML이나 결정적 트리 쓰기를 보장하지 않습니다.
...
**첫 설치 수용 확인.** 쓰기 가능한 폴더에서 새 작업을 열고 `$vibespec`으로 “회의실 예약 기획을 간단히 만들고 `outputs/meeting-room.sot.json`과 `outputs/meeting-room.html`에 저장해줘”라고 요청합니다. 먼저 `node <VibeSpec-스킬-디렉터리>/scripts/doctor.mjs <작업-폴더> --json`을 실행합니다. doctor와 아래 설치본 검증기가 모두 PASS해야 전체 모드 성공입니다.

```text
node <VibeSpec-스킬-디렉터리>/scripts/verify-host-output.mjs outputs/meeting-room.sot.json outputs/meeting-room.html --host <claude-code|cowork|codex-cli|codex-desktop> --record host-acceptance/<host>.json
```

이 검증기는 호스트가 만든 JSON이 구조적으로 유효하고 HTML에 들어 있는 SOT와 정확히 같은지 확인합니다. 저장소 CI는 데스크톱 호스트 자체를 실행하지 못하므로, 각 호스트에서 이 검사를 실행한 뒤에만 그 호스트 수용 완료로 표현합니다. 새 작업에서 VibeSpec이 보이지 않으면 설치 뒤 작업을 다시 열거나 데스크톱 앱을 재시작하세요.
...
**Git은 필수가 아닙니다.** 혼자 쓴다면 뷰어의 되돌리기·히스토리로 현재 편집 세션을 관리하고, 공유는 `*.sot.json` 파일을 아무 방법으로나 건네면 됩니다. 아래는 **여러 명이 함께 편집**하거나 **영구 이력이 필요할 때**의 선택지입니다.
...
- VibeSpec의 Node 스크립트는 **자동 네트워크 요청을 하지 않습니다.** 사용자가 명시한 SOT 경로, 스키마, 뷰어 템플릿만 읽습니다.
- 명시한 출력 경로 또는 사용자가 `--apply`를 실행해 지정한 SOT에만 파일을 씁니다. 브라우저 뷰어도 파일 선택기에서 사용자가 고른 파일에만 저장합니다.
- 뷰어에는 사용자가 클릭할 때만 열리는 [vbspec.com](https://vbspec.com/) 링크가 있으며, SOT 내용은 그곳으로 전송하지 않습니다.
...
│       ├── references/workflows/       # 라우팅 뒤 필요한 모드만 읽는 절차
│       ├── scripts/
│       │   ├── doctor.mjs              # 설치 경로·Node·번들·작업 폴더 사전 점검
│       │   ├── verify-host-output.mjs  # 실제 호스트 JSON/HTML 수용 증거
...
│       ├── package-plugin.mjs          # 테스트/.build/개발 파일을 뺀 최소 번들
...
npm run package:plugin # .dist/vibespec 최소 런타임 번들 생성
```
