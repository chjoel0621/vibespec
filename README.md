...
Pages rebuilds these demos from the current viewer and tracked SOTs/change plans whenever `main` deploys. They open directly in a browser; in Chrome or Edge, you can connect a SOT file and save back to it.

| Experience | Live demo |
| --- | --- |
| Basic editing and linked documents | **Meeting booking:** [Product plan](https://chjoel0621.github.io/vibespec/en/) · [Add-on](https://chjoel0621.github.io/vibespec/en/notif/) · [Integrated](https://chjoel0621.github.io/vibespec/en/map/) · [한국어](https://chjoel0621.github.io/vibespec/) |
| Compare proposed work with release scope | **Neighborly:** [Product plan](https://chjoel0621.github.io/vibespec/flea/en/) · [Review](https://chjoel0621.github.io/vibespec/flea/en/workspace/) · [Integrated](https://chjoel0621.github.io/vibespec/flea/en/map/) |
| Resolve a measurement decision and hand it back to AI | **CRM:** [KPI Measurement Check](https://chjoel0621.github.io/vibespec/crm/en/review/?view=semantic) · [한국어](https://chjoel0621.github.io/vibespec/crm/review/?view=semantic) |
| KPI measurement with complete evidence | **WeekChef:** [KPI Measurement Check](https://chjoel0621.github.io/vibespec/weekchef/en/?view=semantic) · [한국어](https://chjoel0621.github.io/vibespec/weekchef/?view=semantic) |

The CRM demo intentionally keeps one product decision open. Saving an answer does not silently close it: the viewer shows the steps and a copy-ready request for attaching the updated SOT to Claude or Codex so the planning changes can be applied and checked. WeekChef is the passing example generated by the packaged VibeSpec 0.17.0 plugin in a real Claude Code session.

The executable **[Before → Finding → Resolved evaluation case](evaluation/semantic-assurance/cases/meeting-room-no-show/README.md)** shows why a structurally valid meeting-room plan cannot yet measure no-shows, then reaches `measurement: ready` by connecting the QR check-in decision, feature, screen, flow, and event producer.
...
| **KPI Measurement Check** | For new semantic-enabled plans, checks KPI → measurement → event/evidence → feature/screen relationships. It identifies affected KPIs, explains decisions and next steps in user language, and after an answer is saved provides a copy-ready request for handing the SOT back to AI. Existing SOTs remain valid and show no extra tab until explicitly enabled. |
...
Ask something like "turn my product idea into a planning tool" or attach a business plan, and the skill produces a viewer HTML with the SOT JSON embedded. New plans also declare how each KPI will be measured; the HTML includes a derived KPI Measurement Check report. A result that needs a decision or supporting evidence may be delivered as an honest draft, but it is not labeled ready for approval or developer handoff.

**Applying a measurement decision:** answer the question in KPI Measurement Check and choose `Save answer`. The answer is recorded in the SOT, but the decision stays open. Save the JSON from the top bar, then attach it to Claude or Codex with the request shown by the viewer. VibeSpec updates only the affected KPI, event, feature, page, and flow through a reviewed change plan; reruns structural, content, and measurement checks; and marks the decision complete only after those checks pass. It then regenerates the current JSON and HTML.
...
- **For team collaboration, share only the JSON file.** The viewer (HTML) is the app, so you only need to hand it out once. After that, exchange the saved `*.sot.json` file; the other person connects it in the same viewer and sees the **same plan and check results**. No more copying around heavy documents.
...
- **Recommended workflow.** Edit in the viewer → **Save** → replace the `*.sot.json` in your repo → commit / open a PR. Teammates pull and open the file in the same viewer to see the same plan and check results.
