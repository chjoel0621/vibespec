...
1. **입력 수집 및 모드 판별**
   ...
   - **라우팅 규칙**(inspect 결과 기준):
     - `.sot.json`이 없고 아이디어·기획 문서만 있으면 → **신규 생성**(아래 2~4단계, 본편 SOT 1.0).
     - `needsRebase`가 true면 먼저 그 사실을 알리고 → **재기준(rebase)**을 우선 제안한다.
     - main이 있고 요청이 작은 제자리 수정(오타·상태·수용 기준·문구)이면 → **수정 모드**.
     - `suggestedModes`가 `repair`뿐이면(`invalidReason` 참조 — 다중 본편·구조 오류) 생성·수정·이니셔티브를 진행하지 말고 그 오류를 먼저 알리고 고친다.
     - main이 있고 요청이 범위 있는 증분("결제 얹어줘", "검색 개선 이니셔티브")이면 → **이니셔티브 모드**. 새 path는 inspect의 `nextPath`(예 `root→1-3`)를 그대로 쓴다(직접 번호를 지어내지 말 것). ⚠️ 단, `pathAuthority`가 `incomplete`이면(단일 파일만 줬을 때) 형제 이니셔티브를 놓쳐 번호를 재발급할 수 있으니, **제품 폴더 전체를 inspect에 다시 넘겨** `complete` 상태로 만든 뒤 발급한다.
     - "전체를 한눈에/합쳐 보여줘"(편집이 아닌 조망)면 → **제품 지도**.
     - `incompleteTree`가 true면(이니셔티브만 있고 본편이 없음) 트리 작업을 하지 말고 **본편 파일을 요청**한다.
