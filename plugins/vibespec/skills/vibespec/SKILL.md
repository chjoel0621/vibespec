...
4. **접점을 경계 스텁으로 표현한다 — 페이지와 섹션 둘 다.** 증분이 본편의 화면에서 시작·연결되면, 그 접점을 `boundary`가 있는 페이지로 IA에 넣고, **그 페이지를 감싸는 섹션에도 `boundary`를 달아 본편의 어느 섹션인지 밝힌다**:
   ```json
   { "id":"S1", "title":"<본편 섹션 제목 그대로>", "boundary": { "scopeId":"root", "sectionId":"<그 페이지가 든 본편 섹션 S#>" },
     "pages":[
       { "id":"P1", "title":"<본편 페이지 제목 그대로>", "type":"<본편 페이지 타입 그대로>", "refs":[],
         "boundary": { "scopeId":"root", "pageId":"<본편의 그 페이지 P#>" }, "children":[ …이 증분의 화면들… ] }
     ] }
   ```
   - 페이지 스텁의 `title`·`type`은 **본편 페이지를 그대로 미러링**한다(다르면 drift 경고). 스텁은 참조이므로 자체 `refs`를 두지 않는다. 새 화면들은 스텁의 `children`으로 매단다.
   - **섹션 스텁의 `title`은 본편 섹션을 그대로 미러링**하고, `sectionId`는 그 페이지 접점이 든 본편 섹션이다. 이렇게 해야 이니셔티브의 모든 계층이 **참조(미러) 또는 신규** 둘 중 하나로 명시되고, 합성 지도에서 조용히 사라지는 그릇 섹션이 생기지 않는다(감싸는 섹션에 boundary가 없으면 validate-tree가 "참조로 선언하거나 신규임을 명시하라"고 경고한다).
   - 반대로 증분이 본편에 **없던 새 섹션**을 더하는 것이면, 그 섹션엔 `boundary`를 두지 않는다 — 지도에 작성자가 지은 제목 그대로 `+이니셔티브` 태그로 나타난다.
...
3. 지도와 거기서 연 문서는 모두 읽기 전용이다(편집 크롬 숨김, 입력 잠금, 로컬 저장·되돌리기 없음). proposed·dropped·조상 비활성 이니셔티브는 합성에서 빠지고 "제외"로 표시된다. stale implemented는 "기준 낡음"으로 남는다.
4. 지도는 특정 시점의 스냅샷이다 — 편집은 여전히 개별 본편·이니셔티브 파일에서 하고, 필요할 때 지도를 다시 만든다. 사용자에게 지도를 건넬 때 이 점(스냅샷·읽기 전용·편집은 원본에서)을 한 줄로 알린다.

## 병합(merge) — 구현된 이니셔티브를 본편에 접어 넣기

이니셔티브가 실제로 구현되면(`implemented`), 그 증분을 **본편 baseline에 영구 반영**할 수 있다. 지도가 읽기전용 스냅샷을 "그리는" 것과 달리, 병합은 같은 접붙임을 **편집 가능한 본편 SOT 1.0에 실체화**한다(이니셔티브의 id를 본편 공간으로 재번호, 경계 스텁 해소, requirements·flow·inScope 병합). 노드 모델판 "본편에 머지".

1. 본편+이니셔티브가 든 폴더에서 먼저 **드라이런**: `node "<VIBESPEC_SKILL_DIR>/scripts/merge.mjs" "<폴더 절대경로>" --only <이니셔티브 id>`. 무엇이 본편의 어느 id로 접붙고(R/F/P/S 재번호), 어디에 붙는지, PRD 중 수동 검토가 필요한 항목, 머지 후 stale이 되는 형제가 무엇인지 보여준다.
2. 적용: `... merge.mjs "<폴더>" --only <id> --apply`. 본편 파일을 갱신하고, 그 이니셔티브를 `landed`(접힘)로 표시한다(삭제하지 않음 — 이력 보존, 이후 합성·활성집합에서 제외).
3. **적격 조건**(도구가 강제): `implemented`만, **본편 직속**만, **활성 자식이 없어야**(자식부터 land/drop), 트리에 다른 오류가 없어야. 하나라도 어긋나면 거부하고 사유를 알린다.
4. **PRD는 자동으로 다 합치지 않는다.** 이니셔티브의 `inScope`만 본편에 더하고, `problem`·`solution`·`nonGoals`·`goal`은 리포트에 "검토 필요"로 표시한다 — 본편 서사에 사람이 판단해 반영한다.
5. **병합은 본편 변경이라 나머지 이니셔티브가 stale이 된다.** 도구는 이를 리포트만 하고 조용히 고치지 않는다 — 이어서 rebase로 갱신한다(재기준 절차). 사용자에게 "결제 이니셔티브를 본편에 접었고, 남은 N개는 rebase가 필요하다"처럼 명확히 안내한다.
