...
기계 판독용 JSON Schema는 `sot.schema.json`에 있다. `../scripts/validate-sot.mjs`가 이 스키마를 직접 실행하고, 동적 참조와 IA 커버리지를 추가 검증한다. 검증기는 **SOT 1.0과 1.1을 모두** 지원한다. 레거시 파일은 뷰어에서 불러온 뒤 저장하거나 `migrate-sot.mjs`로 정규화한다.
...
- **현재 지원:** `schemaVersion: "1.0"`(제품 기획), `"1.1"`(추가 기획).
- **레거시 입력:** `schemaVersion`이 생략된 파일은 지원되는 레거시다. `node scripts/migrate-sot.mjs old.sot.json --out main.sot.json`을 먼저 드라이런하고, 결과가 검증되면 `--apply`로 **새 파일**에 기록한다.
- **미지원 입력:** 명시된 버전이 `1.0`/`1.1` 이외면 자동 변환하지 않는다. 해당 형식을 파악하거나 별도 migration을 제공하기 전까지 repair 대상으로 취급한다.
- **불변 정책:** 마이그레이터는 뷰어와 같은 정규화 함수를 쓰며, 1.1을 1.0으로 다운그레이드하지 않는다. 모든 migration 결과는 `validate-sot`를 통과해야 한다.
...
- `schemaVersion`: 필수. 일반 SOT는 `"1.0"`, 추가 기획 SOT는 `"1.1"`. 레거시 입력에는 없을 수 있지만, 뷰어가 저장하거나 스킬이 생성하는 SOT에는 반드시 포함한다.
- `lang`: 뷰어 기본 표시 언어(`"ko"`/`"en"`, 선택). 영어로 생성하면 `"en"`.
- 파일은 Git 형상관리를 위해 **키 정렬 고정 + pretty-print**된 `<제품명>.sot.json`으로 저장한다(뷰어의 저장 기능이 이 표준형을 출력한다).

## 추가 기획 (SOT 1.1)

> 추가 기획 생성·재기준·병합은 스킬에 활성화되어 있다. 아래는 그 공통 계약이다.

추가 기획은 제품 기획 SOT를 그대로 두고 그 위에 얹히는 증분 문서다. 구조적으로는 완전한 SOT이고, 여기에 두 가지가 더해진다.

**PRD는 역할별로 다르다(경량 프로파일).** 제품 정체성 필드(`category`·`platforms`·`northStar`·`differentiator`·`alternatives`)는 **제품 기획 소관**이라 추가 기획에는 두지 않는다(내용이 있으면 검증기가 경고 — 드리프트 위험). 추가 기획 PRD의 필수 최소 집합은 `problem`·`solution`·`inScope`·`nonGoals`(오버레이의 핵심 = "X를 더하고 Y는 안 한다")이며, 나머지 스코프 필드(`goal`·`kpis`·`scenarios` 등)는 선택이다. 제품 기획(1.0)은 여전히 전체 6섹션을 요구한다.
...
- `productId`·`id`: 소문자 slug(`^[a-z0-9][a-z0-9-]*$`). `id`는 같은 제품 안에서 유일하며 생성 후 불변. `"root"`는 제품 기획 예약어라 `id`로 못 쓴다.
- `path`: 표시·파일명용 경로(`1-2`, `1-2-1`). 참조 식별자가 아니다.
- `status`: `proposed | approved | implemented | dropped | landed`. `landed`는 구현된 추가 기획을 제품 기획에 접어 넣은(merge) 뒤 남는 종료 상태 — 그 내용은 이미 제품 기획 baseline에 있으므로 합성·활성집합에서 제외되고 digest 신선도도 따지지 않는다(이력 보존용 tombstone). `merge.mjs`가 부여하며, 손으로 지정하지 않는다.
- `parent.scopeId`: 부모 scope(`"root"` 또는 부모 추가 기획의 `id`). 자기 자신은 못 가리킨다.
- `parent.canonicalization`: 항상 `"sot-c14n-v1"`. `parent.digest`: 부모 SOT를 그 규칙으로 표준화한 SHA-256(`sha256:` + 64 hex).

**경계 스텁(boundary).** 추가 기획이 제품 기획의 화면에서 시작·연결될 때, 그 접점을 페이지에 `boundary`로 표시한다. 스텁 페이지는 제품 기획 페이지를 가리키는 참조일 뿐이므로 자체 기능 `refs`를 담지 않는다(담으면 경고).

```json
{ "id": "P1", "title": "장바구니", "type": "top", "refs": [], "boundary": { "scopeId": "root", "pageId": "P10" }, "children": [ … ] }
```

**섹션 경계(section boundary).** 페이지 경계 스텁을 감싸는 섹션도, 제품 기획의 어느 섹션에 해당하는지 `boundary`로 밝힌다. 그래야 추가 기획의 모든 계층이 **참조(제품 기획 미러) 또는 신규** 둘 중 하나로 명시되고, 합성 때 조용히 사라지는 그릇 섹션이 없어진다.

```json
{ "id": "S1", "title": "예약", "boundary": { "scopeId": "root", "sectionId": "S4" }, "pages": [ …페이지 경계 스텁… ] }
```

- 섹션 스텁의 `title`은 대상 제품 기획 섹션을 **그대로 미러링**한다(다르면 drift 경고). `sectionId`는 `^S[1-9][0-9]*$`.
- 섹션에 `boundary`가 **있으면** = 참조: 그 안의 새 페이지는 제품 기획 섹션에 더해지고, 페이지 스텁은 각자 대상 페이지 아래로 접붙는다. 합성 버전에 새 섹션은 생기지 않는다.
- 섹션에 `boundary`가 **없으면** = 추가 기획이 더하는 신규 섹션: 합성 버전에 작성자가 지은 제목 그대로, `+추가 기획` 태그로 나타난다(자동 이름·조용한 삭제 없음).
...
큰 SOT를 AI가 통째로 다시 쓰지 않도록 `vibespec-change-plan-v2`를 사용한다. 계획은 `baseDigest`와 실제 변경 경로 전체(`expected.touchedPaths`)를 선언하고, 적용기는 그 경로가 실제 diff와 정확히 일치할 때만 기록한다. requirement·feature·section·page와 PRD 항목은 typed 연산으로 수정한다.

- 상세기능 `F#:index`는 위치가 참조 의미다. 새 항목은 끝에 append하고, 삭제는 마지막 항목만 허용한다. 수정/삭제 시 현재 `before` 값을 함께 제출한다.
- `boundary`와 `initiative` 메타는 부모 SOT와의 교차 파일 정합성이 필요하므로 단일 파일 변경 계획의 대상이 아니다. 구조 변경 뒤에는 `validate-tree`를 실행한다.
- 변경 계획은 `outputs/`가 아니라 `history/change-plans/`에 보관한다. 적용 시 `--receipt <파일>`로 base digest·결과 digest·변경 경로를 기록한다. 이 계획은 이미 적용된 감사 기록이므로 다시 실행하지 않고, 다음 수정에는 새 계획을 만든다.
