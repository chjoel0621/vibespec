...
common에서 선택한 기획 목적도 적용한다. `change`의 핵심 상세기능은 기존 `desc`와 `acceptance` 안에서 다음 질문에 답한다.

1. 누가 어떤 상태에서 실행하는가? 필요한 권한·전제는 무엇인가?
2. 무엇을 입력·선택하며 어떤 조건에서 거부되는가?
3. 성공 후 무엇이 바뀌고 사용자에게 어떤 결과가 보이는가?
4. 실패·취소·중복·권한 부족·빈 상태 중 해당하는 주요 예외에서 무엇이 유지되거나 바뀌는가?
5. 어떤 입력·상태와 관찰 가능한 결과로 완료를 확인하는가?

모든 기능에 모든 예외를 강제로 추가하지 않는다. 해당 없음의 이유와 미확인 결정을 기록한다. `overview`는 생략 범위를 밝히고, `current-state`는 관찰한 사실과 추정을 구분한다. 자동 리뷰가 이 질문들의 의미적 충족을 모두 판정하지는 않으므로 작성자가 함께 검토한다.
