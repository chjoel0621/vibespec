...
function refreshFileConnectionStatus(){
  ...
  if(CONNECTED_FILE_HANDLE) return setFileStatus("connected",t("연결됨 · ","Connected · ")+connectedFileName());
...
async function restoreConnectedHandle(){
  ...
  try{
    ...
    db.close();
    if(row&&row.handle){ CONNECTED_FILE_HANDLE=row.handle; refreshFileConnectionStatus(); return true; }
...
function applyLoadedSot(text,label){
  try{
    SOT=normalize(JSON.parse(text)); resetSelections(); pushHistory(label||t("파일 다시 불러오기","File reload")); markViewerSaved(); render(); return true;
  }catch(_){
...
async function writeConnectedSot(forceOverwrite){
  ...
  try{
    ...
    CONNECTED_FILE_SIGNATURE=fileTextSignature(next); markViewerSaved(); refreshFileConnectionStatus(); flash(); return true;
  }catch(_){
...
async function chooseSaveLocation(){
  try{
    ...
    CONNECTED_FILE_SIGNATURE=""; await persistConnectedHandle(); refreshFileConnectionStatus();
...
async function reloadConnectedSot(skipUnsavedConfirm){
  if(RO) return false;
  ...
  try{
    const remote=await readConnectedFile();
    if(!applyLoadedSot(remote.text,t("파일 다시 불러오기","File reload"))) return false;
    CONNECTED_FILE_SIGNATURE=remote.signature; refreshFileConnectionStatus(); return true;
  }catch(_){
...
async function connectExistingSot(){
  ...
  try{
    ...
    await persistConnectedHandle(); refreshFileConnectionStatus(); return reloadConnectedSot(true);
  }catch(error){
    if(error&&error.name==="AbortError") return false;
...
function loadFallbackFile(file){
  ...
  rd.onload=()=>{ if(applyLoadedSot(rd.result,t("파일 불러오기","File load"))) setFileStatus("warning",t("다운로드 모드 · 파일이 연결되지는 않았습니다","Download mode · file is not connected")); };
  rd.readAsText(file);
}
