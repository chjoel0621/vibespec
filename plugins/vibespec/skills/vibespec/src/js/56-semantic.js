...
function renderSemanticReview(){
  ...
  const measurementRows=kpis.map(k=>{
    ...
    const status=stale?t("다시 점검 필요","Check again"):!report||report.assessment?.status==="not-assessed"?t("점검 전","Not checked"):affected?t("확인 필요","Needs review"):t("현재 문제 없음","No current issue");
  ...
  return `<div class="semantic-view">
    <div class="sem-title"><div><h1>${t("KPI 측정 점검","KPI Measurement Check")}</h1><p>${t("기획에 선언된 KPI 측정 방식과 근거의 연결을 확인합니다. 실제 성과 수치나 분석 결과의 정확성을 보증하지 않습니다.","Checks the links between declared KPI measurement methods and evidence. It does not verify actual performance values or analytics accuracy.")}</p></div></div>
    <div class="sem-scope"><b>${t("이번 점검 범위","Scope of this check")}</b><p>${t("선언된 측정 근거를 점검한 결과입니다. 구조 검증·기획 내용 리뷰와 별개이며 제품 승인이나 개발 전달 준비 완료를 뜻하지 않습니다. 정책 일관성, 예외 흐름, 구현·배포 상태는 별도로 확인해야 합니다.","This checks declared measurement evidence, separately from structural validation and content review. It is not product approval or development handoff readiness. Policy consistency, exception paths, implementation and deployment require separate review.")}</p></div>
    ...
    ${!stale&&(actionNeeded||(assessment==="passed"&&readiness==="ready"))?nextNotice:""}
